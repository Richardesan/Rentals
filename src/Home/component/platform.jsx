...
const Platform = () => {
  ...
  return (
    <section className="pt-20 bg-primary-light text-black pb-16 h-screen">
      <article>
        <div className="flex items-center justify-center">
          <img src="/Star.png" />
          <p className="text-4xl lato-bold">
            {" "}
            <span className="lato-bold-italic">Why</span> Choose Our Platform
          </p>
          <img src="/Star.png" />
        </div>
        <p className="w-[40vw] text-center mx-auto mt-4 lato-regular text-2xl px-3">
          We've simplified the rental process with features designed for both
          tenants and landlords.
        </p>
      </article>
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 px-6 sm:px-12 lg:px-40 mt-28 pb-10 ">
        {platformList.map((data, index) => {
          ...
          return (
            <article
              key={index}
              className={`flex gap-x-6 items-start pb-3 flex-wrap sm:flex-nowrap p-10 ${
                isNotLastColumn ? "border-r border-r-black/30" : ""
              } ${isNotLastRow ? "border-b border-b-black/30" : ""}`}
            >
              <div className=" min-w-8">
                <img src={data.logo} className="w-full h-auto object-contain" />
              </div>
              <div className="pb-3 w-full">
                <h1 className="text-2xl lato-bold">{data.title}</h1>
                <p className="text-xl lato-regular mt-3 break-words pr-20">
                  {data.text}
                </p>
              </div>
            </article>
